...
def point_add2(point, other):
    ...
    l = ((othery - pointy) * \
         utils.inverse_mod(otherx - pointx, p)) % p

    x3 = (l * l - pointx - otherx) % p
    y3 = (l * (pointx - x3) - pointy) % p

    return x3, y3

def point_mult(p, scalar):
    r = ZERO
    d = [int(x) for x in bin(scalar)[2:]]
    for i in d:
        if i == 1:
            r = point_add(r, p)
        p = point_double(p)
...
if __name__ == "__main__":
    ...
    # print(point_mult(G, 78439))
    # print(point_mult(G, 78439) == res)
    ...
    # print(point_add(res, res2))   
    print(point_add(res, res2) == resadd)
    print(point_add2(res, res2) == resadd)
